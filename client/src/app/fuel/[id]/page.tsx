...
import { ArrowLeft, CheckCircle, Clock, XCircle, Fuel } from 'lucide-react'
...
export default function FuelDetailPage() {
  ...
  const fetchRequest = async () => {
    try {
      ...
      toast.error('Failed to load fuel request')
      router.push('/fuel')
    } finally {
      setFetching(false)
    }
  ...
  const handleMarkServed = async () => {
    ...
    try {
      await api.put(`/fuel/${id}/mark-served`)
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'served':
        return <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded text-sm">Served</span>
      case 'rejected':
  ...
  const canMarkServed =
    (user?.permissions?.canMarkServed || user?.department === 'Finance' || user?.role === 'administrator') ?? false
      ...
      <main className="flex-1 lg:ml-64 p-6">
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            Back to Fuel & Vehicle Requests
          </Link>

          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
            <div className="px-6 py-4 border-b dark:border-gray-700 flex justify-between items-center">
              <h1 className="text-xl font-bold text-gray-900 dark:text-white">Fuel / Vehicle Request Details</h1>
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-sm text-gray-500 dark:text-gray-400">Type of Request</label>
                  <p className="font-medium text-gray-900 dark:text-white">{request.typeOfRequest}</p>
                </div>
                <div>
                  <label className="text-sm text-gray-500 dark:text-gray-400">Date of Request</label>
                ...
                {request.status === 'served' && request.servedAt && (
                  <div className="md:col-span-2">
                    <label className="text-sm text-gray-500 dark:text-gray-400">Served</label>
                    <p className="font-medium text-gray-900 dark:text-white">
                      {format(new Date(request.servedAt), 'PPp')}
                      {request.servedBy && ` by ${request.servedBy.staffId}`}
                    </p>
                  </div>
                )}
              </div>
              ...
              {request.status === 'approved' && canMarkServed && (
