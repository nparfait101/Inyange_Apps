...
export default function PettyCashDetailPage() {
  ...
  const fetchRequest = async () => {
    try {
      const res = await api.get(`/petty-cash/${id}`)
      setRequest(res.data)
    } catch (error) {
      toast.error('Failed to load petty cash request')
  ...
  const handleMarkPaid = async () => {
    setMarkingPaid(true)
    try {
      await api.put(`/petty-cash/${id}/mark-paid`)
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
            <div className="px-6 py-4 border-b dark:border-gray-700 flex justify-between items-center">
              <h1 className="text-xl font-bold text-gray-900 dark:text-white">Petty Cash Request Details</h1>
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-sm text-gray-500 dark:text-gray-400">Names</label>
                  <p className="font-medium text-gray-900 dark:text-white">{request.names}</p>
                </div>
                <div>
                ...
                <div>
                  <label className="text-sm text-gray-500 dark:text-gray-400">Expense Date</label>
                  <p className="font-medium text-gray-900 dark:text-white">
                    {format(new Date(request.expenseDate), 'MMM dd, yyyy')}
                  </p>
                </div>
                <div>
                  <label className="text-sm text-gray-500 dark:text-gray-400">Amount Requested</label>
                  <p className="font-medium text-gray-900 dark:text-white">
                ...
                <div className="md:col-span-2">
                  <label className="text-sm text-gray-500 dark:text-gray-400">Reason of Expense</label>
                  <p className="font-medium text-gray-900 dark:text-white">{request.reasonOfExpense}</p>
                </div>
                {request.bankAccount?.accountNumber && (
                  <div className="md:col-span-2">
                    <label className="text-sm text-gray-500 dark:text-gray-400">Bank Account</label>
                    <p className="font-medium text-gray-900 dark:text-white">
                      {request.bankAccount.accountName} – {request.bankAccount.accountNumber}
                    </p>
                  </div>
                )}
                {request.phoneNumber?.number && (
                  <div>
                    <label className="text-sm text-gray-500 dark:text-gray-400">Phone (with name)</label>
                    <p className="font-medium text-gray-900 dark:text-white">
                      {request.phoneNumber.name} – {request.phoneNumber.number}
                    </p>
                  </div>
                )}
                {request.attachment && (
                  <div className="md:col-span-2">
                    <label className="text-sm text-gray-500 dark:text-gray-400">Attachment</label>
                    <a
