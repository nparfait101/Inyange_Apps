...
export default function PettyCashPage() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      toast.error('Failed to fetch petty cash requests')
    } finally {
      setFetching(false)
    }
  }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'paid':
        return <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded text-xs">Paid</span>
      case 'rejected':
      ...
      <main className="flex-1 lg:ml-64 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Petty Cash Requests</h1>
            <Link
              href="/petty-cash/new"
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm font-semibold text-gray-900 dark:text-white">
                            {new Intl.NumberFormat('en-US', {
                              style: 'currency',
                              currency: 'RWF',
                            }).format(request.amountRequested)}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900 dark:text-white">
